...
import { IChangeStatus, IListNotifications, INotificationQuery, INotificationService, ISaveToken, ISendNotification, ISettings } from './interfaces';
...
export class NotificationService implements INotificationService {
    ...
    async changeStatus(payload: IChangeStatus) {
        try {
            ...
            await this.notificationRepository.findOneAndUpdate(filterQuery, updateOperation)
            return null
        } catch (error) {
            throw error;
        }
    ...
    async sendNotification(payload: ISendNotification) {
        try {
            const { senderId, notificationType, recipientsId, title, message } = payload;
    ...
    private async pushNotification(payload: ISendNotification) {
        try {
            ...
            singleToken ? notificationPayload['token'] = fcmTokens[0] : notificationPayload['tokens'] = fcmTokens;

            await this.firebaseNotificationService[singleToken ? 'sendSinglecast' : 'sendMulticast'](notificationPayload);
            return null
        }
        catch (error) {
            throw error;
